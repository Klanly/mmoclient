...
public class WaterEffect : MonoBehaviour
{
    ...
    public int reflectionTextureSize
    {
        get
        {
            switch (quality)
            {
                ...
                case Quality.Medium: return 256;
    ...
    {
        WaterEffect[] wws = FindObjectsOfType(typeof(WaterEffect)) as WaterEffect[];
        if (wws.Length > 0)
            foreach (WaterEffect ww in wws) ww.quality = q;
    }
    ...
    {
        Clear();
        foreach (DictionaryEntry kvp in mCameras)
