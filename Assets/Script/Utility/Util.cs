...
public class Util
{
    ...
    public static void RefreshLogFlag()
    {
        LogFilter.Instance().Init();
    }
    public static void Log(string flag, string str)
    {
        LogFilter.Instance().Log(flag, str);
    }

    public static void LogWarning(string flag, string str)
    {
        LogFilter.Instance().Warning(flag, str);
    }

    public static void LogError(string flag, string str)
    {
        LogFilter.Instance().Error(flag, str);
    }
    ...
    [NoToLuaAttribute]
    public static object[] CallMethod(LuaTable kLuaClass, string functionName, params object[] pArgs)
    {
        LuaFunction method = Util.GetMethod(kLuaClass, functionName);
        if (method == null)
        {
            return null;
        }
        object[] array = new object[pArgs.Length + 1];
        array[0] = kLuaClass;
        pArgs.CopyTo(array, 1);
        return method.Call(array);
    }

    [NoToLuaAttribute]
    public static LuaFunction GetMethod(LuaTable kLuaClass, string functionName)
    {
        if ((kLuaClass != null) && ((kLuaClass[functionName] != null) && (kLuaClass[functionName].GetType() == typeof(LuaFunction))))
        {
            return (LuaFunction)kLuaClass[functionName];
        }
        return null;
    }
    ...
    public static void SwitchAdvancedPerf()
    {
        AdvancedPerf advPerf = GameObject.FindObjectOfType<AdvancedPerf>();
        advPerf.SwitchAdvancedPerf();
    }

    public static void TestBugly()
    {
        BuglyTest testObj = AppFacade.Instance.gameManager.gameObject.GetComponent<BuglyTest>();
        if (testObj == null)
        {
            AppFacade.Instance.gameManager.gameObject.AddComponent<BuglyTest>();
        }
        else
        {
...
public class UIUtil
{
    ...
    public static LuaBehaviour AddLuaBehavComponent(GameObject go, string luaFullPath)
    {
        LuaBehaviour lua = null;
        if (go != null)
        {
            lua = go.GetComponent<LuaBehaviour>();
            if (lua != null)
            {
                GameObject.DestroyImmediate(lua);
            }
            lua = go.AddComponent<LuaBehaviour>();
            lua.LoadLuaScript(luaFullPath);
        }
        return lua;
    }
