...
public class HeartBeat
{
    ...
    {
        Util.Log("Game", string.Format("开始对时 connection:{0} ", connection.name));
        StopSyncTime();
        if (timer == null)
        {
            SendPingMessage();
    ...
    public void SendPingMessage()       //向客户端时间戳发给服务端
    {
        ...
        LuaMessage.CS_PING message = new LuaMessage.CS_PING();
        message.clienttime = (ulong)GetTimestamp();

        MemoryStream ms = new MemoryStream();
        Serializer.Serialize<LuaMessage.CS_PING>(ms, message);
        connection.Send((int)MESSAGE_OPCODE.CLIENT_MESSAGE_OPCODE_PING, ms.ToArray());
    }
    public void OnPingPack(byte[] data)
    {
        MemoryStream ms = new MemoryStream(data);
        SC_PING_BACK message = Serializer.Deserialize<LuaMessage.SC_PING_BACK>(ms);
        long mClientTime = (long)message.clienttime;
        long mServerTime = (long)message.servertime;
        updateDeltaTime(mClientTime, mServerTime);

        SendPingBackMessage(mServerTime);    //返回服务端时间给服务器
        LastHeartBeatTime = GetTimestamp();
    }
    ...
    public void SendPingBackMessage(long serverTime)       //向服务端返回服务端时间戳
    {
        ...
        long clientTime = GetTimestamp();
        LuaMessage.CS_PING_BACK_BACK message = new LuaMessage.CS_PING_BACK_BACK();
        message.servertime = (ulong)serverTime;
        message.clienttime = (ulong)clientTime;

        MemoryStream ms = new MemoryStream();
        Serializer.Serialize<LuaMessage.CS_PING_BACK_BACK>(ms, message);
        connection.Send((int)MESSAGE_OPCODE.CLIENT_MESSAGE_OPCODE_PING_BACK, ms.ToArray());
        if(!isReady)
        {
            isReady = true;
            Util.Log("Game", string.Format("对时完成 connection:{0} ", connection.name));
        }        
    }
}