...
public class SocketClient
{
    ...
    public void ConnectServer(string host, int port, int timeout)
    {
        ...
        client.NoDelay = true;
        Thread th = null;
        Util.Log("Game", string.Format("begin connect ip={0} port={1}", host, port));
        try
        {
        ...
        {
            Util.Log("Game", "connect failed! msg=" + ex.Message);
    ...
    void StartWaitOne()
    {
        ...
        else
        {
            ...
            {
                Util.Log("Game", string.Format("连接超时"));
    ...
    void OnRead(IAsyncResult asr)
    {
        if (connection.state != ConnectState.Success)
        {
            Util.Log("Game", string.Format("OnRead failed! state={0}", connection.state));
            return;
        }
        if (!client.Connected)
        {
            Util.Log("Game", string.Format("OnRead failed! client.Connected is false"));
            return;
        }
        int bytesRead = 0;
        try
    ...
    void OnMessage(MemoryStream ms)
    {
        ...
        if((MESSAGE_OPCODE)action == MESSAGE_OPCODE.SERVER_MESSAGE_OPCODE_PING_BACK)
        {
            connection.OnPingBack(stream.ToArray());
        }
        else
        {
            connection.Enqueue(action, stream.ToArray());
        }
    }
    ...
    void OnWrite(IAsyncResult r)
    {
        if (connection.state != ConnectState.Success)
        {
            Util.Log("Game", string.Format("OnWrite failed! state={0}", connection.state));
            return;
        }
        if (!client.Connected)
        {
            Util.Log("Game", string.Format("OnWrite failed! client.Connected is false"));
            return;
        }
        try
        {
    ...
    public void SendMessage(int action, byte[] data, int serviceType = 0)
    {
        if(connection.state != ConnectState.Success)
        {
            Util.Log("Game", string.Format("SendMessage failed. state={0}", connection.state));
            return;
        }
    ...
    public void Close()
    {
        ...
        Util.Log("Game", string.Format("socket client close"));
