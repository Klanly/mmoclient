...
public class Connection
{
    ...
    {
        INetMessageHandler moduleHandle = null;
        switch ((MESSAGE_OPCODE)action)
        {
            case MESSAGE_OPCODE.SERVER_MESSAGE_OPCODE_LUA_MESSAGE:
                moduleHandle = AppFacade.Instance.gameManager.GetLuaModule();
                break;
            case MESSAGE_OPCODE.SERVER_MESSAGE_OPCODE_CREATE_ENTITY:
            case MESSAGE_OPCODE.SERVER_MESSAGE_OPCODE_DESTROY_ENTITY:
            case MESSAGE_OPCODE.SERVER_MESSAGE_OPCODE_MOVE:
            case MESSAGE_OPCODE.SERVER_MESSAGE_OPCODE_STOP_MOVE:
            case MESSAGE_OPCODE.SERVER_MESSAGE_FORCE_POSITION:
            case MESSAGE_OPCODE.SERVER_MESSAGE_OPCODE_TURN_DIRECTION:
                moduleHandle = AppFacade.Instance.gameManager.GetAOIModule();
                break;
            case MESSAGE_OPCODE.SERVER_MESSAGE_OPCODE_PING_BACK:
                // 对时单独处理 
                return;
            default:
                if(OnDefaultMessage != null)
                {
                    OnDefaultMessage(action, Encoding.ASCII.GetString(data));
                }
                //Util.LogError("Game", "invalid message action:" + action);
                break;
        }
        ...
        moduleHandle.Handle(this, action, data);
    }
}
